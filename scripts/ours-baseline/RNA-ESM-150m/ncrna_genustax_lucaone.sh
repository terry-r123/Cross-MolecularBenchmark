...
if [ "$USE_SLURM" == "1" ]; then
    ...
    data_root=/home/bingxing2/ailab/group/ai4bio/public/multi-omics/lucaone/DownstreamTasksDataset/dataset/GenusTax/gene/multi_class
...
batch_size=4
data=''
data_file_train=${data_root}/train/train.csv; data_file_val=${data_root}/dev/dev.csv; data_file_test=${data_root}/test/test.csv
MODEL_PATH=/home/bingxing2/ailab/group/ai4bio/public/multi-omics/RNA/model/ours/ESM-RNA/ESM2-MARS-main/esm/esm8m_2parts_5m
OUTPUT_PATH=./outputs/ft/dna-all/${task}/rna/ours-baseline/${MODEL_TYPE}  
...
do
    ...
    do 
        ...
        EXEC_PREFIX="${run_type} --nodes=1 -p ${quotatype} -A ${partition} --job-name=${MODEL_TYPE}_${task}  --gres=gpu:$nproc_per_node --cpus-per-task=4 torchrun --nproc_per_node=$nproc_per_node --master_port=$master_port"           
        echo ${MODEL_PATH}

        ${EXEC_PREFIX} \
        downstream/train_genustax_lucaone.py \
