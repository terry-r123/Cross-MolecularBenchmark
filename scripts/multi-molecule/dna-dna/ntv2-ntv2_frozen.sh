...
task='dna-dna' # omics1: siRNA,  omics2: mRNA
task_type='multi'
...
DATA_PATH=${data_root}multi-omics/multi-omics/downstream/EnhancerPromoter

omics1_model_type='nt_v2_100m_multi'
omics2_model_type='nt_v2_100m_multi'

omics1_token='non-overlap'
omics2_token='non-overlap'
...
omics1_model_max_length=500
omics2_model_max_length=500
lr=1e-5
data=''

OMICS1_MODEL_PATH=${data_root}multi-omics/DNA/model/opensource/nt/${omics2_model_type}
OMICS2_MODEL_PATH=${data_root}multi-omics/DNA/model/opensource/nt/${omics2_model_type}
        
for seed in 42
do
    for lr in 3e-6 1e-5 5e-6 7e-6 #3e-5 5e-5 7e-5
    do
        out_name=${task_type}/${task}/opensource/${omics1_model_type}_${omics2_model_type}_${task}_${seed}_${lr}_frozen
        ...
        ${EXEC_PREFIX} \
        downstream/train_enhancer_promoter_inter.py \
