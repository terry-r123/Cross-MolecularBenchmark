...
class RnaLmSelfAttentionTriton(RnaLmSelfAttention):
    ...
    def forward(
        ...
        #print('4444444444',hidden_states.shape)
        
        qkv = self.Wqkv(hidden_states)
        #print(qkv.shape,indices.shape,max_seqlen_in_batch)
        qkv = pad_input(qkv, indices, cu_seqlens.shape[0] - 1,
                        max_seqlen_in_batch)  # batch, max_seqlen_in_batch, thd
        #print('55555555',qkv.shape)
        qkv = rearrange(qkv,
                        'b s (t h d) -> b s t h d',
                        t=3,
                        h=self.num_attention_heads)
        #print('666666666',qkv.shape)
...
class RnaLmLayer_Triton(nn.Module):
    ...
    def forward(
        ...
        #print('222222222',type(hidden_states))
...
class RnaLmEncoder(nn.Module):
    def __init__(self, config):
        ...
        if self.position_embedding_type == 'alibi':
            ...
            #print("self.current_ALiBi_size",self.current_ALiBi_size)
    ...
    def forward(
        ...
        if self.position_embedding_type == 'alibi':
            ...
            #print('000000000',hidden_states.shape)
            if self.config.attn_implementation=='triton':
                hidden_states, indices, cu_seqlens, _ = unpad_input(
                    hidden_states, attention_mask_bool)
                #print('000001111',hidden_states.shape,indices.shape,cu_seqlens.shape)
            ...
            for i, layer_module in enumerate(self.layer):
                ...
                if self.position_embedding_type == 'alibi' and self.config.attn_implementation=='triton':
                    ...
                    layer_outputs = layer_module(hidden_states,
                                                ...
                                                bias=alibi_attn_mask)
                    hidden_states = layer_outputs[0]
                    #print('999999',hidden_states.shape)
        ...
        if self.position_embedding_type == 'alibi' and self.config.attn_implementation=='triton' and subset_mask is None:
            ...
            #print('101010110',hidden_states.shape)
        #pdb.set_trace()
        # Add last layer
        if output_hidden_states:
            all_hidden_states = all_hidden_states + (hidden_states,)
...
class RnaLmForRegression512concat(RnaLmPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.config = config
    ...
    def forward(
        ...
        # print(f"output12.shape: {output12.shape}")
