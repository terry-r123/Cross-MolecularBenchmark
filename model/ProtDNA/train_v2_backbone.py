...
from nt.nt_v2_100m_multi.modeling_esm import EsmModel
...
from ProtDNA.prodna_dataset_v2 import ProtDNADataset
...
def main(args):
    ...
    # 2. load model
    if "nt_v2_100m_multi" in args.dna_model_name_or_path:
    ...
    elif "DNABERT2" in args.dna_model_name_or_path:
    ...
    try:
        for param in dna_backbone.esm.parameters():
            param.requires_grad = False
    except:
        for param in dna_backbone.bert.parameters():
            param.requires_grad = False

    for param in protein_backbone.esm.parameters():
        param.requires_grad = False
    ...
    for epoch in range(args.num_train_epochs): 
        ...
        print(f"epoch = {epoch}, start training, {accelerator.device}")
        for batch in tqdm(train_loader,  desc=f"Training Epoch {epoch}, rank = {accelerator.device}"):
            optimizer.zero_grad()
            ...
            weight_mask = batch['weight_mask'].to(accelerator.device)
            post_token_length = batch['post_token_length'].to(accelerator.device)
            ...
            reverse_weight_mask = batch['reverse_weight_mask'].to(accelerator.device)
            reverse_post_token_length = batch['reverse_post_token_length'].to(accelerator.device)
            
            # .module
            
            
            dna_feats = dna_backbone(dna_token_input_ids, reverse_input_ids=dna_reverse_token_input_ids,
                                    attention_mask=dna_token_attention_mask, reverse_attention_mask=dna_token_reverse_attention_mask,
                                    weight_mask=weight_mask, post_token_length=post_token_length,
                                    reverse_weight_mask=reverse_weight_mask, reverse_post_token_length=reverse_post_token_length,
                                    )['logits']

            protein_input_ids = batch['protein_input_ids'].to(accelerator.device)
            protein_feats = protein_backbone(protein_input_ids)['logits'].to(accelerator.device)
