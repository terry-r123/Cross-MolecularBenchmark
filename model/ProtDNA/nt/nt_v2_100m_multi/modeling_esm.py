...
class RnaLmForNucleotideLevel(EsmPreTrainedModel):
...
class DNALmForNucleotideLevel(EsmPreTrainedModel):
    # include Degradation and SpliceAI
    def __init__(self, config, tokenizer=None):
        super().__init__(config)
        self.num_labels = config.num_labels
    ...
    def process_input(self, final_input, input_ids, attention_mask, weight_mask, post_token_length):
        ### init mappint tensor
        ori_length = weight_mask.shape[1]
        batch_size = final_input.shape[0]
        cur_length = int(final_input.shape[1])
        ...
        if self.config.token_type == 'single':
            ...
            logits = torch.zeros((batch_size, ori_length, self.num_labels), dtype=final_input.dtype, device=final_input.device)
            ori_input_seqs= [(self.tokenizer.decode(ids, skip_special_tokens=True)).replace(' ','') for ids in input_ids]
            ori_input_single_input = torch.tensor([[self.tokenizer.encode(nucleotide, add_special_tokens=False)[0] for nucleotide in ori_input_seq] for ori_input_seq in ori_input_seqs], dtype=input_ids.dtype, device=final_input.device)
            nucleotide_indices = {nucleotide: (ori_input_single_input == self.tokenizer.encode(nucleotide, add_special_tokens=False)[0]).nonzero() for nucleotide in 'ATCGN'}
    ...
    def forward(
        ...
        weight_mask: Optional[bool] = None,
        post_token_length: Optional[bool] = None,
        reverse_weight_mask: Optional[bool] = None,
        reverse_post_token_length: Optional[bool] = None,
    ) -> Union[Tuple[torch.Tensor], SequenceClassifierOutput]:
        r"""
        labels (`torch.LongTensor` of shape `(batch_size,)`, *optional*):
            Labels for computing the sequence classification/regression loss. Indices should be in `[0, ...,
        ...
        logits = self.process_input(final_input, input_ids, attention_mask, weight_mask, post_token_length)

        reverse_outputs = self.esm(
            reverse_input_ids,
        ...
        reverse_final_input = reverse_outputs[0]
        reverse_logits = self.process_input(reverse_final_input, reverse_input_ids, reverse_attention_mask, reverse_weight_mask, reverse_post_token_length)

        combined_logits = torch.cat([logits, reverse_logits], dim=1)
