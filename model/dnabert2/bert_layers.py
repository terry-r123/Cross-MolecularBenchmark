...
try:
    ...
    # print("Not using flashattention!!!!!!!")
...
class BertEmbeddings(nn.Module):
    ...
    def forward(
        ...
        seq_length = input_shape[1]
        gpu_number = int(input_ids.device.index)
        if use_alibi:
        ...
        #print(use_alibi)
        #print(inputs_embeds.shape,token_type_embeddings.shape,position_embeddings.shape)
...
class BertUnpadSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, cu_seqlens: torch.Tensor,
        ...
        if self.p_dropout or flash_attn_qkvpacked_func is None:
            ...
            # print(f"convert_dtype: {convert_dtype}")
...
class BertEncoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.last_layer_norm = getattr(config, 'last_layer_norm', False)
        if self.last_layer_norm:
            self.last_layer_ln = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
            print('Using last_layer_norm !!!!!!!!!!!!!!!!!!!!')
        else:
            print('NO using last_layer_norm!!!!!!!!!!!!!!!!!')
    ...
    def forward(
        ...
        # gpu_number = int(hidden_states.device.index)
        if use_alibi:
            alibi_attn_mask = attn_bias + alibi_bias # use alibi
        else:
            alibi_attn_mask = attn_bias 
        # print(f"attn_bias = {attn_bias[0]}, {attn_bias.shape}")
        # print(kkk)
        all_encoder_layers = []
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        use_alibi: Optional[bool] = None,
        output_cls_pooled_output: Optional[bool] = True,
        **kwargs
        ...
        embedding_output = self.embeddings(input_ids, token_type_ids,
                                           position_ids, use_alibi=use_alibi)
        ...
        if masked_tokens_mask is None:
            ...
            pooled_output = self.pooler(
                sequence_output, pool=output_cls_pooled_output) if self.pooler is not None else None
        else:
...
class BertLMPredictionHead(nn.Module):
    ...
    def __init__(self, config, bert_model_embedding_weights):
        ...
        factory_kwargs = {'device': bert_model_embedding_weights.device, 'dtype': bert_model_embedding_weights.dtype}
        self.transform = BertPredictionHeadTransform(config)
        # The output weights are the same as the input embeddings, but there is
        # an output-only bias for each token.
        self.decoder = nn.Linear(bert_model_embedding_weights.size(1),
                                 bert_model_embedding_weights.size(0))
        # print(f"init cuda = {bert_model_embedding_weights.device}, seld.decoder={self.decoder}")
        self.decoder.weight = bert_model_embedding_weights
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        hidden_states = self.transform(hidden_states)
        hidden_states = self.decoder(hidden_states)
        # print(f"cuda = {hidden_states.device}, seld.decoder={self.decoder},hidden_states ={hidden_states.shape}")
        # pdb.set_trace()
        return hidden_states
...
class BertForMaskedLM(BertPreTrainedModel):
    ...
    #     print(f"hhhhhhhhhhhhhhhhhhhhhh new_embeddings = {new_embeddings}")
...
class BertForSequenceClassification(BertPreTrainedModel):
    ...
    def __init__(self, config, use_alibi, pos_weight=None, my_vocab_size=None):
        super().__init__(config)
        ...
        if my_vocab_size is not None:
            self.config.vocab_size = my_vocab_size
    ...
    def forward(
        ...
        device = input_ids.device if input_ids is not None else inputs_embeds.device
        pos_weight = torch.tensor([self.pos_weight] * self.num_labels).to(device)
...
class BertForSequenceClassificationReg512Concat(BertPreTrainedModel):
    def __init__(self, config, use_alibi, use_features=True):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.config = config
        self.use_alibi = use_alibi
        ...
        if self.use_features:
            self.classifier = nn.Linear(config.hidden_size*12 +8 , config.num_labels)
        else:
    ...
    def forward(
        ...
        final_input= torch.cat((output1,output2,output3,output4,output5,output6,output7,output8,output9,output10,output11,output12),dim = -1)
...
class BertForSequenceClassificationReg1536Concat(BertPreTrainedModel):
    def __init__(self, config, use_alibi, use_features=True):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.config = config
        ...
        self.dropout = nn.Dropout(classifier_dropout)
        if self.use_features:
            self.classifier = nn.Linear(config.hidden_size*4 +8 , config.num_labels)
        else:
    ...
    def forward(
        ...
        output1 = self.bert(
            input_ids[:,:1536],
            attention_mask=attention_mask[:,:1536],
            token_type_ids=token_type_ids,
            position_ids=position_ids,
            head_mask=head_mask,
            inputs_embeds=inputs_embeds,
        ...
        output2 = self.bert(
            input_ids[:,1536:3072],
            attention_mask=attention_mask[:,1536:3072],
            token_type_ids=token_type_ids,
            position_ids=position_ids,
            head_mask=head_mask,
            inputs_embeds=inputs_embeds,
        ...
        output3 = self.bert(
            input_ids[:,3072:4608],
            attention_mask=attention_mask[:,3072:4608],
            token_type_ids=token_type_ids,
            position_ids=position_ids,
            head_mask=head_mask,
            inputs_embeds=inputs_embeds,
        ...
        output4 = self.bert(
            input_ids[:,4608:],
            attention_mask=attention_mask[:,4608:],
            token_type_ids=token_type_ids,
            position_ids=position_ids,
            head_mask=head_mask,
            inputs_embeds=inputs_embeds,
        ...
        final_input= torch.cat((output1,output2,output3,output4),dim = -1)
        # print(pooled_output.shape)
        if self.use_features:
            final_input = torch.cat(( final_input,features),dim = -1)
        final_input= self.dropout(final_input)
...
class BertForSequenceReg512ConcatNonoverlap(BertPreTrainedModel):
    def __init__(self, config, use_alibi, use_features=True):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.config = config
        ...
        self.dropout = nn.Dropout(classifier_dropout)
        if self.use_features:
            self.classifier = nn.Linear(config.hidden_size*2 +8 , config.num_labels)
        else:
    ...
    def forward(
        ...
        final_input= torch.cat((output1,output2),dim = -1)
        # print(pooled_output.shape)
        if self.use_features:
            final_input = torch.cat(( final_input,features),dim = -1)
        final_input= self.dropout(final_input)
...
class BertForSequenceReg512ConcatStep2(BertPreTrainedModel):
    def __init__(self, config, use_alibi, use_features=True):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.config = config
        ...
        self.dropout = nn.Dropout(classifier_dropout)
        if self.use_features:
            self.classifier = nn.Linear(config.hidden_size*6 +8 , config.num_labels)
        else:
    ...
    def forward(
        ...
        output6 = self.bert(
            input_ids[:,2560:],
            attention_mask=attention_mask[:,2560:],
            token_type_ids=token_type_ids,
            position_ids=position_ids,
            head_mask=head_mask,
            inputs_embeds=inputs_embeds,
        ...
        final_input= torch.cat((output1,output2,output3,output4,output5,output6),dim = -1)
        # print(pooled_output.shape)
        if self.use_features:
            final_input = torch.cat((final_input,features),dim = -1)
        final_input= self.dropout(final_input)
...
class BertForSequenceClassificationReg(BertPreTrainedModel):
    def __init__(self, config, use_alibi, use_features=True):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.config = config
...
class BertForDeepsea(BertPreTrainedModel):
    """Bert Model transformer with a sequence classification/regression head.
    ...
    def __init__(self, config, use_alibi, pos_weight=None):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.config = config
...
class BertForSei(BertPreTrainedModel):
    """Bert Model transformer with a sequence classification/regression head.
    ...
    def forward(
        ...
        # spline_out = self.spline_tr(pooled_output)
        # pdb.set_trace()
        # reshape_out = spline_out.view(spline_out.size(0),  * self._spline_df)
        # logits = self.classifier(reshap)
...
class BertForSequenceRNAdegra(BertPreTrainedModel):
    def __init__(self, config, use_alibi, token_type, tokenizer=None):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.config = config
        self.use_alibi = use_alibi
        ...
        if self.token_type == 'bpe' or  self.token_type=='non-overlap':
    ...
    def forward(
        ...
        elif self.token_type == 'bpe' or  self.token_type=='non-overlap':
        ...
        if self.token_type == '6mer' or self.token_type =='single': 
...
class BertForSei100k(BertPreTrainedModel):
    """Bert Model transformer with a sequence classification/regression head.
...
class BertForRegression(BertPreTrainedModel):
    def __init__(self, config, use_alibi):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.config = config
...
class BertForSequenceClassificationPromptTokenAvg(BertPreTrainedModel):
    """Bert Model transformer with a sequence classification/regression head.
    ...
    def __init__(self, config, use_alibi, my_vocab_size=None, with_prompt_token=0):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.config = config
        if my_vocab_size is not None:
        ...
        self.dropout = nn.Dropout(classifier_dropout)
        self.classifier = nn.Linear(config.hidden_size, config.num_labels)
        self.with_prompt_token = with_prompt_token
        # Initialize weights and apply final processing
        self.post_init()
    ...
    def forward(
        ...
        outputs = self.bert(
            ...
            return_dict=return_dict,
            use_alibi=self.use_alibi,
            output_cls_pooled_output=False,
        )

        pooled_output = outputs[1][:, :self.with_prompt_token+1, :].mean(dim=1)
        pooled_output = self.dropout(pooled_output)
        logits = self.classifier(pooled_output)
...
class BertForSequenceRNAdegra(BertPreTrainedModel):
    def __init__(self, config, use_alibi, tokenizer=None):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.config = config
        self.use_alibi = use_alibi
