...
class SSCNNPredictor(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        if self.args.model_type == "LucaOne":
            if self.is_freeze:
                with torch.no_grad():
                    output = self.extractor(input_ids=input_ids, attention_mask=attention_mask, token_type_ids=data_dict['token_type_ids'])
            else:
                output = self.extractor(input_ids=input_ids, attention_mask=attention_mask, token_type_ids=data_dict['token_type_ids'])
        else:
            if self.is_freeze:
        ...
        elif self.args.model_type == 'ntv2': 
            hidden_states = output[0]
        elif self.args.model_type == 'esm2': 
            hidden_states = output[0]
        elif self.args.model_type == 'esm-1b': 
            hidden_states = output[0]
        elif self.args.model_type == 'BEACON-B': 
            hidden_states = output[0]
        elif self.args.model_type == 'LucaOne': 
            hidden_states = output[0]
