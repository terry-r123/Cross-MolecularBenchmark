...
os.environ["WANDB_PROJECT"] = "cds_beta_lactamase"
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    metric_for_best_model : str = field(default="valid_spearman")
...
class SupervisedDataset(Dataset):
    ...
    def __init__(self, 
        ...
        data = load_from_disk(data_path)[state]

        if len(data.features) == 3:
            # data is in the format of [text, label]
            texts = [d['sequence'].upper().replace("U", "T") for d in data]
...
def compute_metrics(eval_pred):
    ...
    if isinstance(logits, tuple):  # Unpack logits if it's a tuple
        logits = logits[0]
    if logits.ndim == 2:  # Unpack logits if it's a tuple
...
# change for LucaOne
    ...
    if training_args.eval_and_save_results:
        for data_test in data_test_list:
            ...
            test_dataset = SupervisedDataset(tokenizer=tokenizer, args=training_args,
                                     data_path=data_args.data_path,
                                     state=data_args.data_test_path,
                                     kmer=data_args.kmer)
