...
class SupervisedDataset(Dataset):
    ...
    def __init__(self, 
        ...
        if len(data[0]) == 3:
            omics1 = [d[0].upper() for d in data]  # antibody
            omics2 = [d[1].upper() for d in data] # antigen             
...
def train():
    ...
    if any(model_type in training_args.omics2_model_type for model_type in ['esm', 'cnn', 'resnet', 'lstm']):
    ...
    if 'esm' in training_args.omics2_model_type:
        print(f'Loading {training_args.omics2_model_type} model')
        omics2_model =  EsmModel.from_pretrained(
            model_args.omics2_model_name_or_path,
            cache_dir=training_args.cache_dir,
            num_labels=train_dataset.num_labels,
            )
